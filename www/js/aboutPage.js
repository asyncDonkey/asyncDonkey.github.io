...
const competenzeTecniche = [
    {
        ...
        descrizioneDettagliata:
            "Cuore pulsante di ogni interazione, utilizzato per manipolare il DOM, orchestrare chiamate di rete asincrone (con Promises e async/await) e gestire lo stato dell'applicazione in tempo reale.",
        esempioPratico:
            'Tutta la logica interattiva del sito, dal gioco codeDash! alla gestione dei commenti e dei profili utente in `js/main.js` e moduli associati.',
    },
    {
        id: 'html5',
        nome: 'HTML5',
    ...
    },
    {
        ...
        coloreIcona: '#CC6699',
        descrizioneDettagliata:
            "Impiegato per scrivere CSS in modo modulare e manutenibile. L'uso di variabili, nesting, mixin e partials ha permesso di creare un design system coerente e facilmente scalabile.",
        esempioPratico:
            "L'intera cartella `scss/`, dove ogni componente (es. `_header.scss`, `_modals.scss`) è un partial importato nel file principale `styles.scss`.",
    },
    {
        id: 'firestore_db',
        ...
        descrizioneDettagliata:
            "Database NoSQL utilizzato come backend primario per la persistenza di tutti i dati dinamici dell'applicazione, inclusi profili utente, punteggi della leaderboard, articoli e commenti.",
        esempioPratico:
            "Le collezioni `userProfiles`, `leaderboardScores` e `articles` sono esempi centrali dell'architettura dati su Firestore.",
    },
    {
        id: 'firestore_rules',
        nome: 'Security Rules',
        ...
        descrizioneDettagliata:
            "Sviluppo di regole di sicurezza granulari per proteggere i dati su Firestore, garantendo che gli utenti possano leggere e scrivere dati solo secondo i permessi loro concessi. Un pilastro fondamentale dell'applicazione.",
        esempioPratico:
            'Il file `firestore.rules` definisce le logiche di validazione e autorizzazione, ad esempio permettendo a un utente di modificare solo il proprio profilo.',
    },
    {
        id: 'firebase_auth',
        nome: 'Firebase Auth',
        ...
        descrizioneDettagliata:
            "Gestione dell'intero ciclo di vita dell'autenticazione utente: registrazione, login, logout, verifica email. Integrato con le Security Rules per un controllo degli accessi basato sull'identità.",
        esempioPratico:
            "La logica di `onAuthStateChanged` in `js/main.js` e le funzioni `signInWithEmailAndPassword` sono l'implementazione diretta di questo servizio.",
    },
    {
        id: 'cloud_functions',
        ...
        descrizioneDettagliata:
            'Utilizzo di funzioni serverless per eseguire logica di backend in risposta a eventi (es. creazione utente, aggiornamento di un documento) senza gestire un server. Ideale per logiche atomiche e notifiche.',
        esempioPratico:
            'Le funzioni in `functions/index.js`, come quella che assegna i badge al verificarsi di determinate condizioni (`awardAppreciatedAuthorBadge`).',
    },
    {
        id: 'firebase_storage',
        ...
        descrizioneDettagliata:
            "Servizio utilizzato per l'upload e la gestione di file utente, in particolare per gli avatar del profilo. Include la gestione dei metadati per un corretto versioning e cache-busting.",
        esempioPratico:
            "La funzionalità di upload dell'avatar custom in `js/profile.js` che carica l'immagine su Storage e ne salva l'URL in Firestore.",
    },
    {
        id: 'git',
        nome: 'Git & GitHub',
        ...
        descrizioneDettagliata:
            'Utilizzo sistematico di Git per il controllo di versione del codice e di GitHub come repository remoto per la collaborazione, la gestione del progetto (tramite Issues e DevPlan) e il deployment.',
        esempioPratico:
            "L'intero progetto è versionato su GitHub. I commit regolari e l'uso di branch (come `refinement`) dimostrano la padronanza del workflow.",
    },
];
