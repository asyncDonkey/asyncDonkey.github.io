...
C**Struttura Documento Esempio (`/userProfiles/{userId}`):**
...
| `updatedAt`                | timestamp                               | Data e ora dell'ultimo aggiornamento del profilo (per campi come `statusMessage`, `externalLinks`, `bio`, e contatori). | Impostato con `serverTimestamp()` ad ogni modifica permessa.                                                                                                                                                                                                            |
...
| `statusMessage`            | string                                  | Breve messaggio di stato o "stato d'animo" dell'utente.                                                                 | Modificabile dall'utente. Max 150 caratteri. Default: `""`. **Sincronizzato su `userPublicProfiles`**.                                                                                                                                                                  |
...
| `earnedBadges`             | array di string                         | Lista degli ID dei badge guadagnati (riferimento a `badgeDefinitions`).                                                 | Aggiornato dal sistema. Default: `[]`. Valori Esempio: `['verified-user', 'author-rookie', 'the-debuggator', 'glitchzilla-slayer', 'article-commenter', 'guestbook-interactive', 'content-explorer', ...]`. La visualizzazione pubblica è tramite `userPublicProfiles`. |
...
| `avatarUrls`               | map / null                              | Mappa contenente URL alle varie versioni dell'avatar utente.                                                            | Es. `{ original: "url", profile: "url_256x256", small: "url_48x48" }`. Gestito da Cloud Function. **Sincronizzati su `userPublicProfiles`**.                                                                                                                            |
...
| `activeNicknameAnimation`  | string / null                           | **(NUOVO)** Salva la classe CSS dell'animazione per il nickname scelta dall'utente.                                     | Default: `null`. Modificabile dall'utente tramite la UI del profilo. **Sincronizzato su `userPublicProfiles`**.                                                                                                                                                         |
| `articleCommentsWritten`   | number                                  | Conteggio dei commenti scritti dall'utente sotto gli articoli.                                                          | Default: `0`. Aggiornato da Cloud Function.                                                                                                                                                                                                                             |
| `guestbookCommentsWritten` | number                                  | Conteggio dei commenti scritti dall'utente nel guestbook.                                                               | Default: `0`. Aggiornato da Cloud Function.                                                                                                                                                                                                                             |
| `commentedArticleIds`      | array di string                         | Array degli ID degli articoli unici commentati dall'utente.                                                             | Default: `[]`. Usato per badge come "Esploratore di Contenuti". Aggiornato da Cloud Function.                                                                                                                                                                           |
| `likedArticleIds`          | array di string                         | Array degli ID degli articoli unici a cui l'utente ha messo "Mi Piace".                                                 | Default: `[]`. Usato per il badge "Mecenate degli Articoli". Aggiornato da Cloud Function.                                                                                                                                                                              |

---

## 2. `userPublicProfiles` (Dati Pubblici Utente)
...
| `statusMessage`           | string / null | Breve messaggio di stato o "stato d'animo" dell'utente.                                | Opzionale. Sincronizzato da `userProfiles`. Max 150 caratteri.                      |
...
| `activeNicknameAnimation` | string / null | **(NUOVO)** Versione pubblica della classe di animazione attiva.                       | Default: `null`. Sincronizzato da `userProfiles`.                                   |
...
## 13. `testTasksDefinition` (Definizioni dei Task di Test)

**(NUOVA COLLEZIONE)** Contiene le definizioni dei task di test disponibili per gli utenti tester.

**Struttura Documento Esempio (`/testTasksDefinition/{taskId}`):**

| Campo              | Tipo      | Descrizione                                                                | Note e Vincoli                                            |
| :----------------- | :-------- | :------------------------------------------------------------------------- | :-------------------------------------------------------- |
| `taskId`           | string    | (ID del documento) ID univoco del task di test.                            | Generato da Firestore o manualmente.                      |
| `title`            | string    | Titolo breve e descrittivo del task.                                       | Obbligatorio. Max ~150 caratteri.                         |
| `description`      | string    | Descrizione dettagliata dell'obiettivo del test.                           | Obbligatorio. Max ~1000 caratteri.                        |
| `stepsToReproduce` | string    | (Opzionale) Passaggi specifici per eseguire il test (Markdown supportato). | Max ~2000 caratteri.                                      |
| `expectedBehavior` | string    | (Opzionale) Comportamento atteso al termine del test.                      | Max ~1000 caratteri.                                      |
| `area`             | string    | (Opzionale) Area della piattaforma a cui si riferisce il test.             | Es. "Profilo Utente", "DonkeyRunner", "Articoli".         |
| `status`           | string    | Stato del task di test: 'active', 'inactive', 'archived'.                  | Default: 'active'. Gestito dall'admin.                    |
| `priority`         | number    | (Opzionale) Priorità del task (es. 1=Alta, 2=Media, 3=Bassa).              | Per ordinamento.                                          |
| `createdAt`        | timestamp | Data e ora di creazione della definizione del task.                        | Impostato con `serverTimestamp()`.                        |
| `updatedAt`        | timestamp | Data e ora dell'ultimo aggiornamento della definizione.                    | Impostato con `serverTimestamp()` su ogni modifica admin. |
