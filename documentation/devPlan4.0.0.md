...
La piattaforma ha recentemente completato le funzionalità base di notifica in-app e un'analisi delle regole di sicurezza. La **priorità immediata** è la stabilizzazione attraverso l'implementazione delle raccomandazioni per `firestore.rules` e l'aggiornamento della documentazione dello schema (`firestore-schema.md`). Successivamente, lo sviluppo si concentrerà sulla realizzazione delle funzionalità core della nuova piattaforma "Knowledge-on-Demand" (KOD), partendo dalla riprogettazione della homepage e dall'implementazione del sistema di richiesta articoli. Molte funzionalità esistenti verranno mantenute, ma la loro priorità e integrazione saranno rivalutate nel contesto della nuova visione.
...
* [ ] Verificare la struttura di `userProfiles`.
