# DEVELOPMENT PLAN (v4.0.0 - Refocused) 🚀
...
La piattaforma ha recentemente completato un'importante revisione della gestione dei profili utente e delle relative regole di sicurezza Firestore (SEC-RULE-001). La priorità immediata è la completa stabilizzazione attraverso l'implementazione delle restanti raccomandazioni per `firestore.rules`, il completamento della documentazione dello schema (`firestore-schema.md`) e la risoluzione del debito tecnico identificato. Questo getterà solide fondamenta per future evoluzioni.
...
_Stato: Il flusso base di sottomissione/revisione articoli è implementato e rimane centrale._

- [x] A.4.2.6.A (Admin UI): Textarea per `rejectionReason`.

- **Task A.5: Sistema di Notifiche In-App (Base).**
    - _(Le funzionalità base del pannello e pagina "Tutte le Notifiche" sono COMPLETATE)_
    ...
    - [⚠️ ON HOLD] **Sub-task A.5.5: Notifiche per Interazioni Sociali Avanzate (Previsto per futura evoluzione KOD).**
        ...
        - ⚠️ [ON HOLD] Notifiche per like/commenti generici (da rivalutare).
...
_Stato: Le funzionalità di base del profilo sono presenti._
...
- **Task AUTH.3: Miglioramenti Funzionali Pagina Profilo Utente (`profile.html`)**
    ...
    - [x] Link Esterni (AUTH.3.2) - _Vedi SEC-RULE-002 per validazione_.
    ...
    - ➡️ **Sub-task AUTH.3.7: Sistema di Amicizie/Followers.** (Priorità MOLTO BASSA / ON HOLD - Funzionalità più prioritarie per la stabilizzazione attuale).

### Sezione C: Funzionalità Specifiche Giochi/Piattaforma 🎮 & Contenuti Esistenti

_Stato: Donkey Runner e Leaderboard rimangono come feature distintive._
...
- ➡️ **Task C.NEW_FEATURE_HP_ACTIVITY: Visualizzare attività recenti sulla homepage.** (SUPERSEDED - Da rivalutare con futura riprogettazione homepage).
...
- ➡️ **Task D.UI-HP-FEATURED: Rivedere il layout grafico della card dell'articolo in evidenza sulla homepage.** (SUPERSEDED - Da rivalutare con futura riprogettazione homepage).
...
- _(Altri task D come da piano precedente v3.17.10 - non elencati qui per brevità, da rivalutare singolarmente)_

### Sezione Sicurezza & Stabilità (Derivata da ANALYSIS-001.4) 🛡️

_Stato: Ora inclusi nella Fase 1 CRITICA._
...
- _(Vedi inizio devplan per task di implementazione SEC-RULE-001 a SEC-RULE-004 e ANALYSIS-001.4.5)_
...
- _(Eventuale Strategia Mobile sarà parte di un piano futuro)_

### Sezione F: Fase di Perfezionamento Finale (Polish) 💅🎨

_Stato: Da rivalutare dopo il completamento dei task di stabilizzazione._

- [x] F.2.4.3.A.1 (Icone Like/Commenti Homepage).
- [x] F.2.4.3.A.2 (Icone Portal Card Homepage).
- _(Altri task F come da piano precedente v3.17.10 - da rivalutare)_

---

## ✅ TASK COMPLETATI RECENTEMENTE (Focus Notifiche Panel/Page & Rules Analysis - Pre-Pivot KOD) ✅

_(Riferimento a v3.18.3 - Rimane invariato, elenca task completati prima dell'Athena Update e di questa revisione del piano)_
