...
- 🧪 **Test Incrementali e Rigorosi:**
    - **Approccio ai Test:** Oltre a testare le funzionalità dopo ogni modifica significativa, verranno eseguiti cicli di test più completi prima di ogni deploy significativo e una verifica post-deploy per assicurare che tutto funzioni come atteso in ambiente di produzione. Questo approccio mira a bilanciare l'agilità dello sviluppo con la stabilità. L'implementazione di funzionalità server-side (es. Cloud Functions) potrebbe richiedere un processo di test più iterativo e dettagliato durante lo sviluppo stesso della funzione.
    - Testare le funzionalità su diversi browser (Chrome, Firefox, Safari, Edge) e dispositivi (desktop, mobile, tablet, o emulatori).
...
- `[ ]` **Task F.2 (Nuovo): Revamp Grafico Generale.** (Priorità Bassa/Futuro)
    ...
    - `[x]` **Sub-task F.2.4 (Nuovo): Integrazione Libreria Icone Moderna.**
        ...
        - `[ ]` **Sub-task F.2.4.3.A (Nuovo - Dettaglio di F.2.4.3): Revamp Icone UI con Material Symbols.** (Priorità Media-Bassa)
            - `[ ]` Sub-task F.2.4.3.A.1: **Pulsanti Like/Commenti:** Sostituire emoji/SVG con Material Symbols (es. `favorite` / `favorite_border`, `comment` / `chat_bubble_outline`) in `index.html` (card articoli), `view-article.html` (articolo e commenti), `js/comments.js` (guestbook).
            - `[ ]` Sub-task F.2.4.3.A.2: **Icone Sezioni Homepage:** Valutare e sostituire le icone SVG nelle `portal-card` di `index.html` (es. `.Dev` -> `code_blocks`, `CodeDash!` -> `sports_esports`, `Leaderboard` -> `emoji_events`, `Mini Leaderboard` -> `list_alt`, `Feedback` -> `feedback`, `Guestbook` -> `forum`, `Articolo Evidenza` -> `star`).
...
1.  **Task C.2.4 (Donkey Runner): Migliorare leggibilità testi e usabilità form in fullscreen.** (Priorità Media)
2.  **Task A.4.2.6.A (Admin UI): Textarea per `rejectionReason`.** (Priorità Media)
3.  **Task D.9: Migliorare UX per Utente Non Registrato.** (Priorità Media)
4.  **Task F.2.4.3.A (Revamp Icone UI):** Iniziare con la sostituzione delle icone più impattanti (es. Like/Commenti, Sezioni Homepage, Pulsanti Condivisione Base). (Priorità Media-Bassa, ma può essere fatto gradualmente)
    - `[ ]` Sub-task F.2.4.3.A.1: Pulsanti Like/Commenti.
    - `[ ]` Sub-task F.2.4.3.A.7: Pulsanti Condivisione Articolo (base).
    - `[ ]` Sub-task F.2.4.3.A.2: Icone Sezioni Homepage (portal-card).
5.  **Task D.1.A.1 (Nuovo): Notifiche Toast per Like.** (Priorità Media-Bassa)
6.  **Task C.5: Migliorare UI/UX di Donkey Runner Game (iniziare con C.5.1, C.5.2).** (Priorità Media)
7.  **Task AUTH.3.4 (Opzionali Profilo): Iniziare con Mini-Bio Utente.** (Priorità Media)
8.  **Task D.3.B.6: Revisionare e aggiornare `README.md`.** (Priorità Media)
9.  **Task C.3: Miglioramenti Leaderboard & Info Glitchzilla.** (Priorità Media)
10. **Task A.5 (Nuovo): Sistema di Notifiche In-App (Base - iniziare con A.5.1, A.5.2).** (Priorità Media-Bassa)
11. **Task AUTH.3.5 (Nuovo): Articoli Preferiti / Segnalibri.** (Priorità Bassa)
12. **Task A.4.4 (Nuovo): Statistiche Utente Aggregate per Admin (iniziare con A.4.4.2 - solo UI base, implementazione logica con CF posticipata).** (Priorità Bassa/Futuro)
13. **Task C.2.8 (Donkey Runner): Risolvere problemi file audio mancanti.** (Priorità Bassa)
14. **Task D.3.B (Documentazione): Riprendere JSDoc e commenti inline/modulo.** (Priorità Continua/Bassa per recupero pregresso)
15. **Task E.3 (Ricerca App Mobile) e E.4.4 (Decisione Implementazione Cloud Functions):** (Priorità Bassa/Ricerca e Decisione a Lungo Termine)
16. **Task D.4: Test e QA (Quality Assurance) - Cicli Pre/Post Deploy.** (Priorità Continua/Finale)
