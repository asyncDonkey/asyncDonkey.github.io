...
**Data Ultimo Aggiornamento:** 22 Maggio 2025 (Athena Update - Completati Task D.9.6 e AUTH.3.6.2)
...
- ✅ **ANALYSIS-001.4.5 (Azione Utente):** Investigare e risolvere attivamente il problema di caricamento di `firestore.rules` nell'emulatore. _(Completato il 22 Maggio 2025)_

✅ **Task SCHEMA-001: Aggiornare `firestore-schema.md` (Priorità ALTA)** _(Completato il 22 Maggio 2025)_
...
- `[x]` Verificare la struttura di `articles` e relative sottocollezioni (es. `comments`, `likes`). _(Nessuna modifica necessaria allo schema esistente)_
- `[x]` Verificare la struttura delle notifiche. _(Proposto e confermato aggiornamento per includere `updatedAt`, allineare `relatedItemId`, e rimuovere `userId` ridondante)_
- **Verifica Necessaria:** Confronto con dati Firestore reali e regole. _(Eseguita durante la sessione)_

---

## 💅 MIGLIORAMENTI STRUTTURALI E UI/UX 💅

✅ **Task CSS-REFACTOR-001: Refactoring CSS con SCSS (Priorità ALTA)** _(Completato il 21 Maggio 2025)_
...
✅ **Task UI-FIX-001: Correzione Visualizzazione Dati Utente in Mini-Leaderboards (Priorità ALTA)** _(Completato il 21 Maggio 2025)_
...
✅ **Task D.9: Migliorare UX per Utente Non Registrato. (Priorità MEDIA)** _(Parzialmente Completato, Task D.9.6 completato il 22 Maggio 2025)_
...
- **Descrizione:** Migliorare l'esperienza utente per i visitatori non autenticati, fornendo feedback chiari e inviti all'azione appropriati.
      - `[x]` **D.9.1 (Visualizzazione Dati Pubblici):** Modificate Firestore Rules per `/userPublicProfiles` (`allow read: if true;`) per consentire la lettura dei dati pubblici (avatar, nickname) agli utenti non autenticati. Verificato il funzionamento in homepage, leaderboard, commenti articoli/guestbook. _(Completato il 22 Maggio 2025)_
      - `[x]` **D.9.2 (Accesso Pagina Profilo):** Modificato `js/profile.js` e `profile.html` per mostrare un messaggio informativo e un invito al login/registrazione quando un utente non autenticato tenta di visualizzare il profilo di un altro utente (invece di un errore di permessi). _(Completato il 22 Maggio 2025)_
      - `[x]` **D.9.3 (Toast Notifications per Azioni Bloccate):**
          - `[x]` Implementata toast notification (e apertura modale login) per like ad articoli (`js/articleViewer.js`) per utenti non autenticati. _(Completato il 22 Maggio 2025)_
          - `[x]` Implementata toast notification (solo toast) per like a commenti articoli (`js/articleViewer.js`) per utenti non autenticati. _(Completato il 22 Maggio 2025)_
          - `[x]` Implementata toast notification (solo toast) per like a commenti guestbook (`js/comments.js`, copre anche Donkey Runner) per utenti non autenticati. _(Completato il 22 Maggio 2025)_
          - `[x]` Implementata toast notification (e apertura modale login) per upvote a issue/suggerimenti (`js/issueTracker.js`) per utenti non autenticati. _(Completato il 22 Maggio 2025)_
      - `[x]` **D.9.4 (Messaggio Interattivo Submit Article):** Migliorato il messaggio "login richiesto" in `submit-article.html` per includere link diretti a `register.html` (con parametri `authAction`). Verificato che `js/register.js` gestisca i parametri. _(Completato il 22 Maggio 2025)_
  ...
      - ✅ **D.9.6 (Messaggio Interattivo Contribute - Form Invio Issue):** Migliorare il messaggio "login richiesto" nel form di invio issue in `contribute.html` per includere link diretti a `register.html` (con parametri `authAction`), simile a D.9.4. _(Completato il 22 Maggio 2025 - AthenaDev)_

🆕 **Task UI-ICON-UPDATE-001: Aggiornamento Icone con Material Symbols (Priorità BASSA)** _(Parzialmente Completato il 22 Maggio 2025)_

- **Descrizione:** Sostituire emoji o icone testuali con Material Symbols per coerenza visiva.
      - `[x]` **UI-ICON-UPDATE-001.1 (Upvote Issue):** Sostituita icona upvote in `js/issueTracker.js` con `how_to_vote` (Material Symbols) e implementata logica per stato filled/empty. _(Completato il 22 Maggio 2025)_
      - `[x]` **UI-ICON-UPDATE-001.2 (Contribute.html Headers):** Sostituite icone testuali/emoji negli `<h3>` di `contribute.html` e nel link "Guida Markdown" con Material Symbols (`ink_pen`, `markdown`, `emoji_objects`, `live_help`). _(Completato il 22 Maggio 2025)_
      - `[ ]` **UI-ICON-UPDATE-001.3 (Altre Sezioni):** Valutare altre aree del sito per aggiornamenti icone.

🆕 **Task UI-GB-STYLE-001: Rivedere stile commenti nel Guestbook (Priorità MEDIA-BASSA)** _(Introdotto il 22 Maggio 2025)_
...
- **Task AUTH.3: Miglioramenti Funzionali Pagina Profilo Utente (`profile.html`)**
  ...
          - ✅ **Sub-task AUTH.3.6.2: Nickname Dinamico - Icona Autore.** _(Completato (escluso header) il 22 Maggio 2025 - AthenaDev)_
...
- **Task C.3: Miglioramenti Leaderboard & Info Glitchzilla** (Priorità MEDIA)
  ...
      - `[P]` Restanti miglioramenti funzionali o di UI per la leaderboard. _(Inizieremo da qui nella prossima sessione)_
...
- `[ ]` Sub-task D.1.A.1: Aggiungere notifiche toast per conferma/errore like (su azioni di like/unlike _effettive_ degli utenti loggati, se non già coperte da feedback UI immediato). (Priorità MEDIA-BASSA).
- _(Task D.9 è stato spostato nella sezione "MIGLIORAMENTI STRUTTURALI E UI/UX" e D.9.6 è COMPLETATO)_
...
## ✅ TASK COMPLETATI RECENTEMENTE (Focus Notifiche Panel/Page & Rules Analysis - Pre-Pivot KOD & Refactoring CSS & UX Guest & Icona Autore) ✅
...
- ✅ **D.9.6 (Messaggio Interattivo Contribute - Form Invio Issue):** Migliorato messaggio "login richiesto". - COMPLETATO (22 Maggio 2025 - AthenaDev)
- ✅ **AUTH.3.6.2 (Icona Autore accanto al Nickname):** Implementata icona 'school' per autori in profilo, articoli, commenti e leaderboard (escluso header). - COMPLETATO (22 Maggio 2025 - AthenaDev)

---

// DevPlan v4.0.1 - Aggiornato e firmato da AthenaDev 🏛️✨
// A domani per continuare con Task C.3!
