...
- 📄 **Documentazione Continua ed Esaustiva:**
    ...
    - Mantenere aggiornata la documentazione di progetto, inclusi lo schema del database (`firestore-schema.md`), le Regole Firestore commentate, il `README.md`, il presente Piano di Sviluppo e documenti di valutazione come `cloudFunctionsEvaluation.md`.
...
- **Task A.4: Miglioramenti Dashboard Admin (`admin-dashboard.html`)**
    ...
    - `[ ]` **Task A.4.4 (Nuovo): Statistiche Utente Aggregate per Admin.** (Priorità Bassa/Futuro)
        - `[x]` Sub-task A.4.4.1: Ricerca su come aggregare dati da Firestore in modo efficiente (query dirette o Cloud Functions per conteggi periodici). _(Valutazione Cloud Functions completata, vedi E.4)_
...
- `[ ]` **Task A.5 (Nuovo): Sistema di Notifiche In-App (Base).** (Priorità Media-Bassa)
    ...
    - `[ ]` Sub-task A.5.2: Logica in `adminDashboard.js` per creare notifiche quando un articolo viene approvato o respinto. _(Valutare se spostare a Cloud Functions per robustezza, vedi E.4)_
...
- `[x]` **Task E.4 (Nuovo): Valutazione Implementazione Cloud Functions per Funzionalità Avanzate.** _(Ricerca Iniziale e Valutazione Completate)_
    - `[x]` Sub-task E.4.1: Analisi requisiti per notifiche utente complesse (es. articolo condiviso, nuovo badge), aggiornamenti contatori aggregati, e potenziale propagazione dati avatar. _(Completato in `documentation/cloudFunctionsEvaluation.md`)_
    - `[x]` Sub-task E.4.2: Stima complessità e valutazione competenze per lo sviluppo di Cloud Functions in Node.js/TypeScript. _(Completato in `documentation/cloudFunctionsEvaluation.md`)_
    - `[x]` Sub-task E.4.3: Valutazione costi potenziali associati a Cloud Functions (Firebase/Google Cloud). _(Completato in `documentation/cloudFunctionsEvaluation.md`)_
    - `[ ]` Sub-task E.4.4: Decisione sull'adozione e pianificazione implementazione per funzionalità specifiche. _(Prossimo passo per questo task)_
...
- `[ ]` **Task F.2 (Nuovo): Revamp Grafico Generale.** (Priorità Bassa/Futuro)
    ...
    - `[x]` **Sub-task F.2.4 (Nuovo): Integrazione Libreria Icone Moderna.**
        ...
        - `[~]` Sub-task F.2.4.3: Sostituzione graduale delle icone SVG personalizzate esistenti e utilizzo della nuova libreria per future necessità, ove appropriato (Iniziato con la navbar).
        - `[ ]` **Sub-task F.2.4.3.A (Nuovo): Completare sostituzione emoji/SVG con Material Symbols per pulsanti Like, Commenti e altre azioni UI.** (Priorità Media-Bassa)
...
- **Notifiche (A.5) e Statistiche Admin (A.4.4):** Aree di crescita future, potenzialmente con Cloud Functions (vedi E.4).
...
- **Implementazione Cloud Functions (vedi Task E.4 e `documentation/cloudFunctionsEvaluation.md`):**
    - **Valutazione Competenze:** L'implementazione di Cloud Functions (tipicamente in Node.js o TypeScript per Firebase) richiederebbe un apprendimento dedicato di questi ambienti server-side, della gestione delle dipendenze (npm/yarn), e delle best practice per funzioni serverless.
    - **Probabilità di Successo Iniziale:** La stima precedente di medio-bassa (40-50%) per implementare Cloud Functions _complesse_ senza un periodo di studio e sperimentazione dedicato rimane valida. Tuttavia, con un approccio incrementale (iniziando con funzioni semplici) e le risorse di apprendimento identificate, la competenza può essere costruita.
    - **Alternative per Notifiche/Aggiornamenti Avatar:**
        - **Notifiche "Il tuo articolo è stato condiviso":** Molto complesso senza logica server. Si potrebbe tracciare solo se la condivisione avviene tramite un pulsante _interno_ al sito che fa una chiamata a Firestore (ma non traccerebbe condivisioni dirette del link).
        - **Aggiornamento Avatar (Task C.1):** La visualizzazione dell'avatar non richiede Cloud Functions. Cloud Functions sarebbero utili per _processare_ l'immagine all'upload (resize, ottimizzazione), come dettagliato in `cloudFunctionsEvaluation.md`.
...
1.  **Task D.4: Test e QA (Quality Assurance) - Continuazione.** (Priorità Alta)
    - `[ ]` D.4.1: Finalizzare definizione casi di test per le funzionalità corrette da BUGFIX-001, per la nuova navbar e per i link ai profili utente.
    - `[ ]` D.4.2: Eseguire secondo ciclo di test manuali focalizzato su Bug Report, Feature Request, Issue Tracker, Navbar, e link ai profili.
    - `[ ]` D.4.3: Test specifici esperienza mobile (generale e Donkey Runner), inclusa la nuova navbar.
2.  **Task D.11: Revisione Contenuto e Layout Pagina 'About' (`about.html`).** (Priorità Media)
    - `[ ]` Sub-task D.11.1: Rileggere e migliorare la chiarezza e l'impatto dei testi.
    - `[ ]` Sub-task D.11.2: Analizzare e correggere i problemi di layout mobile.
3.  **Task C.2.4 (Donkey Runner): Migliorare leggibilità testi e usabilità form in fullscreen.** (Priorità Media)
4.  **Task A.4.2.6.A (Admin UI): Textarea per `rejectionReason`.** (Priorità Media)
5.  **Task D.9: Migliorare UX per Utente Non Registrato.** (Priorità Media)
6.  **Task F.2.4.3.A (Icone Pulsanti): Completare sostituzione emoji/SVG con Material Symbols per pulsanti Like, Commenti e altre azioni UI.** (Priorità Media-Bassa)
...
16. **Task D.3.B (Documentazione): Riprendere JSDoc e commenti inline/modulo.** (Priorità Continua/Bassa per recupero pregresso)
17. **Task E.3 (Ricerca App Mobile) e E.4.4 (Decisione Implementazione Cloud Functions):** (Priorità Bassa/Ricerca e Decisione a Lungo Termine)
