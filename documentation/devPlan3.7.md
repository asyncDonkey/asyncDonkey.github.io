# DEVELOPMENT PLAN (v3.7 - Post-Firestore Rules Admin)

**Stato del Progetto (secondo v3.6 e successivi affinamenti):** Implementati i like per articoli, commenti articoli e commenti guestbook (con visualizzazione likers). Editor Markdown EasyMDE funzionante in `submit-article.html`. Pagina `contribute.html` creata e linkata. Pagina `admin-dashboard.html` creata con logica per visualizzare articoli pending e modale di modifica; funzionalità di approvazione/rigetto/modifica articoli da parte dell'admin implementata e funzionante con le regole Firestore V5.1.
...
* `[x]` **Sub-task A.1.1:** Creare la pagina HTML `contribute.html` con contenuto informativo iniziale.
    * *Contenuto Iniziale (dettagli da v3.4):*
...
* `[ ]` **Sub-task A.2.1:** Definire la struttura dati in Firestore per le "Issues" (es. collezione `userIssues`).
    * *Campi previsti (combinazione v3.6 & v3.4):* `title`, `description`, `type` (gameIssue, generalFeature, newGameRequest), `gameId` (se `type` è gameIssue), `submittedBy` (userId, userName, userNationalityCode), `timestamp`, `status` (new, underConsideration, accepted, planned, inProgress, completed, declined), `upvotes`, `upvotedBy` (array di UID).
    * *Specifiche Tipi Issue (da v3.4):*
...
* `[x]` **Sub-task A.3.1:** Raffinare e finalizzare le regole Firestore per `articles` (workflow `draft`, `pendingReview`, `published`).
    * *Obiettivo (da v3.4):* Solo gli autori possono modificare le proprie bozze. Solo i moderatori/admin possono cambiare lo status da `pendingReview` a `published` o respingere. *(Nota: Regole admin per approva/respingi/modifica implementate e funzionanti come da Task A.4. Regole per autori che modificano bozze sono TODO - vedi Task A.4.2).*
* `[x]` **Sub-task A.3.2:** Sviluppare un'interfaccia admin/moderatore in `admin-dashboard.html` per la gestione degli articoli.
    ...
    * `[x]` Assegnare `publishedAt: serverTimestamp()` quando un articolo viene pubblicato (specifica da v3.4, implementato in `js/adminDashboard.js` e permesso dalle regole Firestore).
...
* `[x]` **Sub-task A.3.3:** Implementare la logica JS per l'interfaccia admin (caricamento articoli, azioni di approva/respingi/modifica, apertura/gestione modale di modifica con EasyMDE, salvataggio modifiche admin).
...
### Task A.4: Implementazione e Debug Regole Firestore per Moderazione Articoli Admin
* `[x]` **Sub-task A.4.1:** Definizione e iterazione di regole Firestore per le operazioni admin (approvazione, rigetto, modifica) sugli articoli.
    * *Dettaglio:* Sviluppate e testate molteplici versioni (V1-V5.1) delle regole Firestore per `articles` (`allow update`) per bilanciare sicurezza e funzionalità per gli amministratori.
    * *Stato Attuale:* Le regole V5.1 (la versione corrente funzionante) consentono agli admin di approvare articoli (cambiando `status` a `published` e impostando `publishedAt`), respingere articoli (cambiando `status` a `rejected`), e modificare il contenuto degli articoli (`title`, `contentMarkdown`, `tags`, `snippet`, `coverImageUrl`, `isFeatured`) mantenendo l'integrità dei campi non modificabili (`authorId`, `createdAt`) e assicurando la corretta gestione di `updatedAt`.
    * *Criticità Risolta:* I problemi di "Missing or insufficient permissions" per le operazioni di approvazione, rigetto e modifica da parte dell'admin sono stati risolti con l'adozione e l'affinamento delle regole V5.1.
* `[ ]` **Sub-task A.4.2:** Implementare regole Firestore per permettere agli autori di modificare i propri articoli in stato `draft` o `rejected` e sottometterli nuovamente per revisione (`pendingReview`).
    * *Stato:* TODO. Questo richiederà di aggiungere una condizione `||` alla regola `allow update` per gli articoli, specificando i permessi per `request.auth.uid == resource.data.authorId`.
...
* **Task B.4: Contribuzione Articoli da Parte degli Utenti (con Moderazione)**
    ...
    * *NOTA (da v3.6/v3.4):* L'ex Sub-task B.4.3 (logica backend per workflow articoli) è stato **SPOSTATO e RINOMINATO in Task A.3.1 e A.4**.
    * *NOTA (da v3.6/v3.4):* L'ex Sub-task B.4.4 (meccanismo approvazione/moderazione) è stato **SPOSTATO e RINOMINATO in Task A.3.2, A.3.3 e A.4**.
* `[x]` **Task B.5:** Gestione Dati Articoli da Firestore (Migrazione da Statico)
    * *Nota:* La visualizzazione degli articoli sulla homepage (`index.html`) tramite `js/homePageFeatures.js` filtra correttamente per `status: "published"`.
...
* *NOTA (da v3.4):* Task C.5 è stato **RIMOSSO** (La funzionalità di tracciamento issue ora è il Task A.2, più completo e integrato).
...
* `[x]` **Sub-task D.6.2:** Migliorare l'aspetto e l'interattività dei bottoni "Like" e dei relativi conteggi nei commenti (articoli e guestbook), inclusa l'indicazione di cliccabilità per visualizzare i likers. *(Nota da v3.4: Parzialmente completato, UX migliorata).*
...
## Considerazioni Chiave e Miglioramenti Futuri Possibili (Combinate da v3.6, v3.4 e affinamenti recenti)

* **Admin Roles / Firestore Rules:** Fondamentale per Task A.2.5 e A.3/A.4. Il sistema per identificare gli utenti amministratori/moderatori (campo `isAdmin: true` in `userProfiles`) è in uso e le regole Firestore lo sfruttano. Le regole sono state significativamente affinate (fino alla V5.1 per gli articoli) per garantire sicurezza e funzionalità corrette per le operazioni admin. È cruciale continuare a testarle e adeguarle per ogni nuova interazione.
    * **Suggerimento:** Per le regole Firestore, specialmente quelle complesse come `allow update` per gli articoli, è utile mantenere commenti dettagliati direttamente nel file `.rules` per spiegare la logica di ogni condizione. Quando si apportano modifiche, testare ogni caso d'uso (approvazione, rigetto, modifica contenuto, modifica da autore) individualmente.
...
* **Modularità del Codice JavaScript:** Con l'aggiunta di nuove pagine e funzionalità, continuare a valutare la possibilità di creare file JS dedicati (es. `contributePage.js`, `issueTracker.js`) e raggruppare le interazioni con Firestore in moduli dedicati (es. `firestoreServiceArticles.js`).
...
* **Sviluppo e Debug:**
    ...
    * Sfruttare meglio il debugger del browser e il simulatore di regole Firestore.
