...
- `[x]` **Sub-task A.3.1:** Raffinare e finalizzare le regole Firestore per `articles` (workflow `draft`, `pendingReview`, `published`).
    - _Obiettivo (da v3.4):_ Solo gli autori possono modificare le proprie bozze. Solo i moderatori/admin possono cambiare lo status da `pendingReview` a `published` o respingere. _(Nota: Regole admin per approva/respingi/modifica implementate e funzionanti come da Task A.4. Regole per autori che modificano bozze sono TODO - vedi Task A.4.2)._
...
- `[x]` **Sub-task A.4.1:** Definizione e iterazione di regole Firestore per le operazioni admin (approvazione, rigetto, modifica) sugli articoli.
    - _Dettaglio:_ Sviluppate e testate molteplici versioni (V1-V5.1) delle regole Firestore per `articles` (`allow update`) per bilanciare sicurezza e funzionalità per gli amministratori.
    - _Stato Attuale:_ Le regole V5.1 (la versione corrente funzionante) consentono agli admin di approvare articoli (cambiando `status` a `published` e impostando `publishedAt`), respingere articoli (cambiando `status` a `rejected`), e modificare il contenuto degli articoli (`title`, `contentMarkdown`, `tags`, `snippet`, `coverImageUrl`, `isFeatured`) mantenendo l'integrità dei campi non modificabili (`authorId`, `createdAt`) e assicurando la corretta gestione di `updatedAt`.
    - _Criticità Risolta:_ I problemi di "Missing or insufficient permissions" per le operazioni di approvazione, rigetto e modifica da parte dell'admin sono stati risolti con l'adozione e l'affinamento delle regole V5.1.
- `[ ]` **Sub-task A.4.2:** Implementare regole Firestore per permettere agli autori di modificare i propri articoli in stato `draft` o `rejected` e sottometterli nuovamente per revisione (`pendingReview`).
    - _Stato:_ TODO. Questo richiederà di aggiungere una condizione `||` alla regola `allow update` per gli articoli, specificando i permessi per `request.auth.uid == resource.data.authorId`.
...
- **Task B.4: Contribuzione Articoli da Parte degli Utenti (con Moderazione)**
    ...
    - _NOTA (da v3.6/v3.4):_ L'ex Sub-task B.4.3 (logica backend per workflow articoli) è stato **SPOSTATO e RINOMINATO in Task A.3.1 e A.4**.
    - _NOTA (da v3.6/v3.4):_ L'ex Sub-task B.4.4 (meccanismo approvazione/moderazione) è stato **SPOSTATO e RINOMINATO in Task A.3.2, A.3.3 e A.4**.
- `[x]` **Task B.5:** Gestione Dati Articoli da Firestore (Migrazione da Statico)
    - _Nota:_ La visualizzazione degli articoli sulla homepage (`index.html`) tramite `js/homePageFeatures.js` filtra correttamente per `status: "published"`.
