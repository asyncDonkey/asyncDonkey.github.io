...
import {
    ...
    POWERUP_DURATION,
    POWERUP_COLORS,
    POWERUP_TARGET_HEIGHT,
    POWERUP_TARGET_WIDTH,
} from './powerUps.js';
...
const projectileColor = '#0ff';
let enemies = [];
let enemyBaseSpawnTimer = 0;
let nextEnemyBaseSpawnTime = 0;
const enemyBaseSpawnColor = '#0f0';
let flyingEnemies = [];
...
class Player {
    ...
    deactivatePowerUp() {
        ...
        if (this.activePowerUp === POWERUP_TYPE.FIREWALL) {
            this.isFirewallActive = false;
        }
        if (this.activePowerUp === POWERUP_TYPE.BLOCK_BREAKER) {
            this.isBlockBreakerActive = false;
        }
...
class Obstacle {
    ...
    draw() {
        ...
        if (this.animation && spriteUsable) {
            ...
            ctx.strokeStyle = obstacleSpawnColor;
...
class BaseEnemy {
    ...
    loadAnimation(spriteNameKeyToLoad, frameW, frameH, numFramesAnim, animationKey) {
        ...
        if (spriteInstance && spriteInstance.complete && spriteInstance.naturalWidth > 0 && numFramesAnim > 0) {
            ...
            console.warn(
                `BaseEnemy.loadAnimation FALLITO per sprite key '${spriteNameKeyToLoad}' (anim key: ${animationKey}).`
            );
    ...
    draw() {
        ...
        if (this.maxHealth > 1 && this.health > 0 && this.health < this.maxHealth) {
            const hbW = this.width * 0.8,
                hbH = 5,
                hbX = this.x + (this.width - hbW) / 2,
                hbY = this.y - hbH - 3;
            ctx.fillStyle = 'rgba(100,100,100,0.7)';
            ctx.fillRect(hbX, hbY, hbW, hbH);
            ctx.fillStyle = 'rgba(0,255,0,0.7)';
            ctx.fillRect(hbX, hbY, hbW * (this.health / this.maxHealth), hbH);
        }
    }
    takeDamage(dmg = 1) {
        this.health -= dmg;
...
class ArmoredShootingEnemy extends BaseEnemy {
    constructor(x, y) {
        super(
            ...
            armoredShootingEnemySpawnColor,
            60
        );
...
function spawnObstacleIfNeeded(dt) {
    ...
    if (obstacleSpawnTimer >= nextObstacleSpawnTime) {
        ...
        obstacles.push(new Obstacle(canvas.width, canvas.height - groundHeight - OBSTACLE_TARGET_HEIGHT));
    }
}
...
function updateShootCooldown(dt) {
    if (!canShoot) {
        shootTimer += dt;
        if (shootTimer >= shootCooldownTime) canShoot = true;
    }
}
...
function spawnEnemyBaseIfNeeded(dt) {
    ...
    if (enemyBaseSpawnTimer >= nextEnemyBaseSpawnTime) {
        ...
        enemies.push(
            new BaseEnemy(
                canvas.width,
                canvas.height - groundHeight - ENEMY_ONE_TARGET_HEIGHT,
...
function spawnFlyingEnemyIfNeeded(dt) {
    ...
    if (flyingEnemySpawnTimer >= nextFlyingEnemySpawnTime) {
        ...
        const localFlyingEnemyY = minY + Math.random() * (maxY - minY);
        flyingEnemies.push(new FlyingEnemy(canvas.width, localFlyingEnemyY));
    }
}
...
function spawnFastEnemyIfNeeded(dt) {
    if (activeMiniboss || score < SCORE_THRESHOLD_FAST_ENEMY) return;
    fastEnemySpawnTimer += dt;
    if (fastEnemySpawnTimer >= nextFastEnemySpawnTime) {
        fastEnemySpawnTimer = 0;
        nextFastEnemySpawnTime = calculateNextGenericEnemySpawnTime(4.0, 7.0);
        fastEnemies.push(
            new BaseEnemy(
                canvas.width,
                canvas.height - groundHeight - ENEMY_TWO_TARGET_HEIGHT,
...
function spawnArmoredEnemyIfNeeded(dt) {
    if (activeMiniboss || score < SCORE_THRESHOLD_ARMORED_ENEMY) return;
    armoredEnemySpawnTimer += dt;
    if (armoredEnemySpawnTimer >= nextArmoredEnemySpawnTime) {
        armoredEnemySpawnTimer = 0;
        nextArmoredEnemySpawnTime = calculateNextGenericEnemySpawnTime(7.0, 12.0);
        armoredEnemies.push(new ArmoredEnemy(canvas.width, canvas.height - groundHeight - ENEMY_THREE_TARGET_HEIGHT));
    }
}
function spawnShootingEnemyIfNeeded(dt) {
    if (activeMiniboss || score < SCORE_THRESHOLD_SHOOTING_ENEMY) return;
    shootingEnemySpawnTimer += dt;
    if (shootingEnemySpawnTimer >= nextShootingEnemySpawnTime) {
        shootingEnemySpawnTimer = 0;
        nextShootingEnemySpawnTime = calculateNextGenericEnemySpawnTime(8.0, 14.0);
        shootingEnemies.push(new ShootingEnemy(canvas.width, canvas.height - groundHeight - ENEMY_FOUR_TARGET_HEIGHT));
    }
}
function spawnArmoredShootingEnemyIfNeeded(dt) {
    if (activeMiniboss || score < SCORE_THRESHOLD_ARMORED_SHOOTING_ENEMY) return;
    ...
    if (armoredShootingEnemySpawnTimer >= nextArmoredShootingEnemySpawnTime) {
        ...
        nextArmoredShootingEnemySpawnTime = calculateNextGenericEnemySpawnTime(15.0, 25.0);
        armoredShootingEnemies.push(
            new ArmoredShootingEnemy(canvas.width, canvas.height - groundHeight - ENEMY_SIX_TARGET_HEIGHT)
        );
    }
}
function spawnToughBasicEnemyIfNeeded(dt) {
    if (activeMiniboss || score < SCORE_THRESHOLD_TOUGH_BASIC_ENEMY) return;
    toughBasicEnemySpawnTimer += dt;
    if (toughBasicEnemySpawnTimer >= nextToughBasicEnemySpawnTime) {
        toughBasicEnemySpawnTimer = 0;
        nextToughBasicEnemySpawnTime = calculateNextGenericEnemySpawnTime(5.0, 9.0);
        toughBasicEnemies.push(
            new ToughBasicEnemy(canvas.width, canvas.height - groundHeight - ENEMY_SEVEN_TARGET_HEIGHT)
        );
    }
}
function spawnDangerousFlyingEnemyIfNeeded(dt) {
    if (activeMiniboss || score < SCORE_THRESHOLD_DANGEROUS_FLYING_ENEMY) return;
    ...
    if (dangerousFlyingEnemySpawnTimer >= nextDangerousFlyingEnemySpawnTime) {
        ...
        nextDangerousFlyingEnemySpawnTime = calculateNextGenericEnemySpawnTime(20.0, 35.0);
        const minY = canvas.height * 0.2;
        const maxY = canvas.height * 0.5;
        const spawnY = minY + Math.random() * (maxY - minY);
        dangerousFlyingEnemies.push(new DangerousFlyingEnemy(canvas.width, spawnY));
    }
}
...
function spawnPowerUpAmbientIfNeeded(dt) {
    ...
    if (powerUpSpawnTimer >= nextPowerUpSpawnTime) {
        ...
        if (asyncDonkey && asyncDonkey.activePowerUp) {
            ...
            if (asyncDonkey.activePowerUp === POWERUP_TYPE.SHIELD)
                availablePowerUps = availablePowerUps.filter((type) => type !== POWERUP_TYPE.SHIELD);
            if (asyncDonkey.activePowerUp === POWERUP_TYPE.FIREWALL)
                availablePowerUps = availablePowerUps.filter((type) => type !== POWERUP_TYPE.FIREWALL);
        }
...
function drawPowerUpItems() {
    powerUpItems.forEach((p) => p.draw(ctx));
}
...
function updateAllEnemyTypes(dt) {
    ...
    flyingEnemies.forEach((e, i) => {
        e.update(dt);
        if (e.x + e.width < 0) flyingEnemies.splice(i, 1);
    });
    fastEnemies.forEach((e, i) => {
        e.update(dt);
        if (e.x + e.width < 0) fastEnemies.splice(i, 1);
    });
    armoredEnemies.forEach((e, i) => {
        e.update(dt);
        if (e.x + e.width < 0) armoredEnemies.splice(i, 1);
    });
    shootingEnemies.forEach((e, i) => {
        e.update(dt);
        if (e.x + e.width < 0) shootingEnemies.splice(i, 1);
    });
    enemyProjectiles.forEach((ep, i) => {
        ep.update(dt);
        if (ep.x + ep.width < 0) enemyProjectiles.splice(i, 1);
    });
    armoredShootingEnemies.forEach((e, i) => {
        e.update(dt);
        if (e.x + e.width < 0) armoredShootingEnemies.splice(i, 1);
    });
    toughBasicEnemies.forEach((e, i) => {
        e.update(dt);
        if (e.x + e.width < 0) toughBasicEnemies.splice(i, 1);
    });
    dangerousFlyingEnemies.forEach((e, i) => {
        e.update(dt);
        if (e.x + e.width < 0) dangerousFlyingEnemies.splice(i, 1);
    });
    if (activeMiniboss) {
...
async function handleSaveDonkeyScore() {
    ...
    if (finalScore <= 0) {
        console.error(
            'handleSaveDonkeyScore: Tentativo di salvare un punteggio non valido (<=0). Punteggio:',
            finalScore
        );
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                userNameToSave =
                    ...
                    (currentUser.email ? currentUser.email.split('@')[0] : 'Utente Registrato');
    ...
    console.log('handleSaveDonkeyScore: Fine funzione.');
}
...
function processGameOver() {
    ...
    if (shouldShowDonkeyScoreInput(finalScore)) {
        if (scoreInputContainerDonkey && playerInitialsDonkeyInput && saveScoreBtnDonkey && restartGameBtnDonkey) {
            ...
            console.warn(
                'processGameOver: Uno o più elementi DOM (scoreInputContainerDonkey, playerInitialsDonkeyInput, saveScoreBtnDonkey, restartGameBtnDonkey) sono NULL. Impossibile mostrare UI per salvare.'
            );
            if (restartGameBtnDonkey) restartGameBtnDonkey.style.display = 'block';
        }
    } else {
        console.log(
            'processGameOver: Punteggio finale non valido o zero, non mostro UI per salvare. Punteggio:',
            finalScore
        );
...
function checkCollisions() {
    ...
    if (!asyncDonkey.isShieldActive) {
        const checkPlayerCollisionWithEntity = (entity, isObstacle = false) => {
            if (isObstacle && asyncDonkey.isFirewallActive) {
                if (
                    entity &&
                    pC.x < entity.x + entity.width &&
                    pC.x + pC.width > entity.x &&
                    pC.y < entity.y + entity.height &&
                    pC.y + pC.height > entity.y
                ) {
            ...
            if (
                ...
                AudioManager.stopMusic();
                console.error(`HIT Player vs ${entity.constructor.name || 'Unknown Entity'}`);
                return true;
            }
            return false;
        };
        ...
        if (checkPlayerCollisionWithList(enemies)) return;
        if (checkPlayerCollisionWithList(fastEnemies)) return;
        if (checkPlayerCollisionWithList(armoredEnemies)) return;
        if (checkPlayerCollisionWithList(shootingEnemies)) return;
        if (checkPlayerCollisionWithList(enemyProjectiles)) return;
        if (checkPlayerCollisionWithList(armoredShootingEnemies)) return;
        if (checkPlayerCollisionWithList(toughBasicEnemies)) return;
    ...
    for (let i = powerUpItems.length - 1; i >= 0; i--) {
        ...
        if (
            ...
            asyncDonkey.activatePowerUp(pu.type);
            powerUpItems.splice(i, 1);
        }
    }
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
            ...
            if (e && p.x < e.x + e.width && p.x + p.width > e.x && p.y < e.y + e.height && p.y + p.height > e.y) {
                ...
                if (e.health <= 0) {
                    ...
                    if (actuallyDangerousFlyer) {
                        ...
                        const randomType =
                            availablePowerUpsDrop[Math.floor(Math.random() * availablePowerUpsDrop.length)];
                    ...
                    e instanceof ArmoredEnemy ||
                    e instanceof ArmoredShootingEnemy ||
                    e instanceof ToughBasicEnemy ||
                    (e instanceof Glitchzilla && e.health > 0)
                ) {
                    score += 5;
                }
                return true;
        ...
        if (
            activeMiniboss &&
            p.x < activeMiniboss.x + activeMiniboss.width &&
            p.x + p.width > activeMiniboss.x &&
            p.y < activeMiniboss.y + activeMiniboss.height &&
            p.y + p.height > activeMiniboss.y
        ) {
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
            if (processProjectileHitOnList(enemies, j)) {
                break;
            }
        }
        if (projectileConsumedThisHit) {
        ...
        for (let j = fastEnemies.length - 1; j >= 0; j--) {
            if (processProjectileHitOnList(fastEnemies, j)) {
                break;
            }
        }
        if (projectileConsumedThisHit) {
        ...
        for (let j = armoredEnemies.length - 1; j >= 0; j--) {
            if (processProjectileHitOnList(armoredEnemies, j)) {
                break;
            }
        }
        if (projectileConsumedThisHit) {
        ...
        for (let j = shootingEnemies.length - 1; j >= 0; j--) {
            if (processProjectileHitOnList(shootingEnemies, j)) {
                break;
            }
        }
        if (projectileConsumedThisHit) {
        ...
        for (let j = armoredShootingEnemies.length - 1; j >= 0; j--) {
            if (processProjectileHitOnList(armoredShootingEnemies, j)) {
                break;
            }
        }
        if (projectileConsumedThisHit) {
        ...
        for (let j = toughBasicEnemies.length - 1; j >= 0; j--) {
            if (processProjectileHitOnList(toughBasicEnemies, j)) {
                break;
            }
        }
        if (projectileConsumedThisHit) {
        ...
        for (let j = dangerousFlyingEnemies.length - 1; j >= 0; j--) {
            if (processProjectileHitOnList(dangerousFlyingEnemies, j)) {
                break;
            }
        }
        if (projectileConsumedThisHit) {
        ...
        for (let j = flyingEnemies.length - 1; j >= 0; j--) {
            if (flyingEnemies[j]) {
                if (processProjectileHitOnList(flyingEnemies, j)) {
                    break;
                }
            } else {
                /* console.warn(`DEBUG: flyingEnemies[${j}] è undefined.`);*/
            }
        }
        if (projectileConsumedThisHit) {
            projectiles.splice(i, 1);
...
function drawTerminalBackgroundEffects() {
    ...
    gradient.addColorStop(0.6, 'rgba(65, 29, 49, 0.3)');
...
function drawPlayingScreen() {
    ...
    if (asyncDonkey && asyncDonkey.activePowerUp && asyncDonkey.powerUpTimer > 0) {
        const powerUpColor =
            POWERUP_COLORS && POWERUP_COLORS[asyncDonkey.activePowerUp]
                ? POWERUP_COLORS[asyncDonkey.activePowerUp]
                : PALETTE.BRIGHT_TEAL;
        ctx.fillStyle = powerUpColor;
        ctx.font = '18px "Source Code Pro", "Courier New", Courier, monospace';
        ctx.textAlign = 'left';
        let powerUpDisplayName =
            POWERUP_THEMATIC_NAMES[asyncDonkey.activePowerUp] || asyncDonkey.activePowerUp.replace(/_/g, ' ');
...
function gameLoop(timestamp) {
    ...
    if (!resourcesInitialized) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText('Loading Resources...', canvas.width / 2, canvas.height / 2);
