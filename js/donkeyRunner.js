...
// ... (other enemy sprite constants remain the same) ...
...
const SHOOTING_ENEMY_PROJECTILE_SOUND = 'enemy_shoot_light.mp3'; // NUOVO: Potenziale suono
...
const ENEMY_PROJECTILE_TARGET_WIDTH = ENEMY_PROJECTILE_ACTUAL_FRAME_WIDTH * GLOBAL_SPRITE_SCALE_FACTOR;
const ENEMY_PROJECTILE_TARGET_HEIGHT = ENEMY_PROJECTILE_ACTUAL_FRAME_HEIGHT * GLOBAL_SPRITE_SCALE_FACTOR;
...
const ENEMY_SIX_SRC = 'images/enemySixPlaceholder.png';
const ENEMY_SIX_ACTUAL_FRAME_WIDTH = 64; const ENEMY_SIX_ACTUAL_FRAME_HEIGHT = 80;
const ENEMY_SIX_IDLE_NUM_FRAMES = 4; const ENEMY_SIX_SHOOTING_NUM_FRAMES = 2;
...
const ARMORED_SHOOTING_ENEMY_PROJECTILE_SOUND = 'enemy_shoot_heavy.mp3'; // NUOVO: Potenziale suono


const ENEMY_SEVEN_SRC = 'images/enemySevenPlaceholder.png';
...
// NUOVO: Miniboss Glitchzilla
const GLITCHZILLA_SPRITE_SRC = 'images/glitchzilla_sprite.png'; // DA CREARE
const GLITCHZILLA_ACTUAL_FRAME_WIDTH = 96; // Esempio
const GLITCHZILLA_ACTUAL_FRAME_HEIGHT = 96; // Esempio
const GLITCHZILLA_NUM_FRAMES = 4; // Esempio
const GLITCHZILLA_TARGET_WIDTH = GLITCHZILLA_ACTUAL_FRAME_WIDTH * GLOBAL_SPRITE_SCALE_FACTOR * 1.2; // Un po' più grande
const GLITCHZILLA_TARGET_HEIGHT = GLITCHZILLA_ACTUAL_FRAME_HEIGHT * GLOBAL_SPRITE_SCALE_FACTOR * 1.2;
const GLITCHZILLA_HEALTH = 25; // Esempio di vita alta
const GLITCHZILLA_SCORE_VALUE = 500;
const GLITCHZILLA_SPAWN_SCORE_THRESHOLD = 2000; // Punteggio per farlo apparire
...
// NUOVO: Mappatura per nomi tematici dei PowerUp
...
const soundsToLoad = [
    ...
    // NUOVO: Suoni per nemici che sparano (DA CREARE)
    ...
    // NUOVO: Suoni per Glitchzilla (DA CREARE)
...
const imagesToLoad = [
    ...
    { name: 'enemyFourProjectile', src: ENEMY_PROJECTILE_SPRITE_SRC },
    ...
    { name: 'enemySix', src: ENEMY_SIX_SRC },
    { name: 'enemySeven', src: ENEMY_SEVEN_SRC },
    { name: 'dangerousFlyingEnemy', src: DANGEROUS_FLYING_ENEMY_SRC },
    // NUOVO: Immagine Glitchzilla
    { name: 'glitchzilla', src: GLITCHZILLA_SPRITE_SRC },
];
...
// NUOVO: Array per Minibosses
let activeMiniboss = null; // Solo un miniboss attivo alla volta per semplicità iniziale
...
function loadImage(name,src){return new Promise((resolve,reject)=>{const img=new Image();images[name]=img;img.onload=()=>{imagesLoadedCount++;console.log(`Img ${name}(${imagesLoadedCount}/${imagesToLoad.length})`);if(imagesLoadedCount===imagesToLoad.length){allImagesLoaded=true;}resolve(img);};img.onerror=()=>{console.error(`ERRORE caricamento: ${name} da ${src}`);resolve(null);};img.src=src;});}
...
async function loadAllAssets() {
    ...
    if(allImagesLoaded) console.log("TUTTE le immagini dichiarate in imagesToLoad sono state caricate con successo.");
...
class Player {
    ...
    activatePowerUp(type) {
        ...
        } else if (this.activePowerUp && this.activePowerUp !== type) { // If has any powerup and picks a different one
             if (exclusiveTypes.includes(type) || // if new one is exclusive OR
                 (this.activePowerUp === POWERUP_TYPE.SHIELD && type !== POWERUP_TYPE.SHIELD) || // old was shield and new is not
                 (this.activePowerUp === POWERUP_TYPE.FIREWALL && type !== POWERUP_TYPE.FIREWALL) // old was firewall and new is not
             ) {
                this.deactivatePowerUp();
             }
        }
        ...
        switch (type) {
            case POWERUP_TYPE.TRIPLE_SHOT:
                ...
                this.isBlockBreakerActive = false; // Ensure other exclusives are off
    ...
    activateSmartBomb() {
        ...
        if(activeMiniboss){ // NUOVO: Smart bomb danneggia il miniboss
            activeMiniboss.takeDamage(5); // Esempio: 5 danni al miniboss
    ...
    constructor(x, y, isUpgraded = false) {
        ...
        if (this.sprite && this.sprite.complete && this.sprite.naturalWidth > 0 && PLAYER_PROJECTILE_NUM_FRAMES > 1) {
            this.animation = new Animation(this.sprite, PLAYER_PROJECTILE_ACTUAL_FRAME_WIDTH, PLAYER_PROJECTILE_ACTUAL_FRAME_HEIGHT, PLAYER_PROJECTILE_NUM_FRAMES, PLAYER_PROJECTILE_ANIMATION_SPEED);
             ...
             console.warn(`Sprite proiettile (${this.isUpgraded ? 'potenziato' : 'normale'}) non completamente caricato o rotto, animazione non creata.`);
        }
    }
...
class BaseEnemy{
    constructor(x,y,targetW,targetH,spriteName,frameW,frameH,numFrames,speedMult,hp=1,fallbackColor='#ccc', scoreValue = 25){ // MODIFICATO: Aggiunto scoreValue
        this.x=x;this.y=y;this.width=targetW;this.height=targetH;
        this.speed=gameSpeed*speedMult;this.health=hp;this.maxHealth=hp;
        this.sprite=images[spriteName];this.fallbackColor=fallbackColor;
        this.animation=null;
        this.numFrames = numFrames;
        this.scoreValue = scoreValue; // MODIFICATO: Memorizza il punteggio
        if(this.sprite && this.sprite.complete && this.sprite.naturalWidth>0 && numFrames>0){
            ...
            else if (numFrames > 0) console.warn(`Sprite ${spriteName} (${this.sprite.src}) per BaseEnemy non caricato o rotto. Animazione non creata.`);
        }
    }
    update(dt){this.x-=this.speed*dt;if(this.animation)this.animation.update(dt);}
    draw(){
...
class ArmoredEnemy extends BaseEnemy{
    constructor(x,y){
        super(x,y,ENEMY_THREE_TARGET_WIDTH,ENEMY_THREE_TARGET_HEIGHT,'enemyThreeBase',ENEMY_THREE_ACTUAL_FRAME_WIDTH,ENEMY_THREE_ACTUAL_FRAME_HEIGHT,ENEMY_THREE_NUM_FRAMES,armoredEnemySpeedMultiplier,ARMORED_ENEMY_HEALTH,armoredEnemySpawnColor, 50); // MODIFICATO: Punteggio
        this.animations={'3':this.animation};
        let dmg1Sprite = images['enemyThreeDmg1'];
        if (dmg1Sprite && dmg1Sprite.complete && dmg1Sprite.naturalWidth > 0) { this.animations['2'] = new Animation(dmg1Sprite,ENEMY_THREE_ACTUAL_FRAME_WIDTH,ENEMY_THREE_ACTUAL_FRAME_HEIGHT,ENEMY_THREE_NUM_FRAMES); }
        else { this.animations['2'] = null; console.warn("Sprite enemyThreeDmg1 non caricato per ArmoredEnemy.");}
        let dmg2Sprite = images['enemyThreeDmg2'];
        if (dmg2Sprite && dmg2Sprite.complete && dmg2Sprite.naturalWidth > 0) { this.animations['1'] = new Animation(dmg2Sprite,ENEMY_THREE_ACTUAL_FRAME_WIDTH,ENEMY_THREE_ACTUAL_FRAME_HEIGHT,ENEMY_THREE_NUM_FRAMES); }
        else { this.animations['1'] = null; console.warn("Sprite enemyThreeDmg2 non caricato per ArmoredEnemy.");}
        this.animation = this.animations[String(this.health)] || this.animation;
    }
    draw(){ //Sovrascritto per gestire cambio sprite corretto
        let animKey = String(Math.max(1, this.health)); // Assicura che la chiave sia almeno '1'
        let currentAnim = this.animations[animKey];

        if (!currentAnim && this.health <= 0) { // Se sconfitto e non c'è animazione per '0' or '1' (se l'ultimo sprite è per health=1)
             // Non disegnare nulla o logica di esplosione qui
        } else if (!currentAnim) { // Fallback all'animazione base se quella specifica non esiste (es. per health > 3)
            currentAnim = this.animations['3'] || this.animation;
        }

        const spriteToDraw = currentAnim ? currentAnim.spritesheet : (this.sprite && this.sprite.complete && this.sprite.naturalWidth > 0 ? this.sprite : null) ;
        const frameWidthToUse = currentAnim ? currentAnim.frameWidth : ENEMY_THREE_ACTUAL_FRAME_WIDTH;
        const frameHeightToUse = currentAnim ? currentAnim.frameHeight : ENEMY_THREE_ACTUAL_FRAME_HEIGHT;

        if(currentAnim && spriteToDraw && spriteToDraw.complete && spriteToDraw.naturalWidth > 0){
            const f=currentAnim.getFrame();
            ctx.drawImage(spriteToDraw,f.sx,f.sy,f.sWidth,f.sHeight,this.x,this.y,this.width,this.height);
        } else if (this.sprite && this.sprite.complete && this.sprite.naturalWidth > 0) {
            ctx.drawImage(this.sprite, 0,0, frameWidthToUse, frameHeightToUse, this.x,this.y,this.width,this.height);
        } else {
            ctx.strokeStyle=this.fallbackColor;ctx.lineWidth=lineWidth;ctx.strokeRect(this.x,this.y,this.width,this.height);
        }
        // Health bar
    ...
    takeDamage(dmg=1){
        ...
        const animKey = String(Math.max(1, this.health)); // Chiave per l'animazione non deve essere < 1
        const newAnim = this.animations[animKey];

        if(newAnim){ this.animation = newAnim; if(this.animation.reset) this.animation.reset(); }
        else if (this.health > 0 && this.animations['1']) { this.animation = this.animations['1']; if(this.animation.reset) this.animation.reset(); }
         else { // Se la vita è <= 0 e non c'è un'animazione specifica, usa quella base o ultima valida
            this.animation = this.animations['1'] || this.animations['3'] || this.animation;
        }
    }
}
...
class ShootingEnemy extends BaseEnemy {
    constructor(x, y) {
        super(x, y, ENEMY_FOUR_TARGET_WIDTH, ENEMY_FOUR_TARGET_HEIGHT, 'enemyFourIdle', ENEMY_FOUR_ACTUAL_FRAME_WIDTH, ENEMY_FOUR_ACTUAL_FRAME_HEIGHT, ENEMY_FOUR_IDLE_NUM_FRAMES, 0.5, 1, shootingEnemySpawnColor, 40); // MODIFICATO: Punteggio
        this.idleAnimation = this.animation;
        this.shootingSprite = images['enemyFourShooting'];
        this.shootingAnimation = null;
        if (this.shootingSprite && this.shootingSprite.complete && this.shootingSprite.naturalWidth > 0) {
            this.shootingAnimation = new Animation(this.shootingSprite, ENEMY_FOUR_ACTUAL_FRAME_WIDTH, ENEMY_FOUR_ACTUAL_FRAME_HEIGHT, ENEMY_FOUR_SHOOTING_NUM_FRAMES, ANIMATION_SPEED / ENEMY_FOUR_SHOOTING_NUM_FRAMES);
        } else { console.warn(`Sprite enemyFourShooting (${this.shootingSprite ? this.shootingSprite.src : 'N/A'}) non caricato per ShootingEnemy.`);}
        this.isShooting = false;
        this.shootTimer = Math.random() * SHOOTING_ENEMY_SHOOT_INTERVAL + 1.5;
        this.shootAnimTimer = 0;
        this.shootAnimDuration = this.shootingAnimation ? (this.shootingAnimation.numFrames / (1/this.shootingAnimation.animationSpeed)) : 0.5; // Corretto il calcolo della durata
    }
    update(dt) {
        this.x -= this.speed * dt;
        let currentAnim = this.isShooting && this.shootingAnimation ? this.shootingAnimation : this.idleAnimation;
        if (currentAnim) { currentAnim.update(dt); }
        if (this.isShooting) {
            this.shootAnimTimer += dt;
            if (this.shootAnimTimer >= this.shootAnimDuration) { this.isShooting = false; if(this.idleAnimation) this.idleAnimation.reset(); } // Resetta anim idle
        } else {
            this.shootTimer += dt;
            if (this.shootTimer >= SHOOTING_ENEMY_SHOOT_INTERVAL) {
                this.shootTimer = 0; this.isShooting = true;
                if (this.shootingAnimation) this.shootingAnimation.reset();
                this.shootAnimTimer = 0;
                enemyProjectiles.push(new EnemyProjectile(this.x - ENEMY_PROJECTILE_TARGET_WIDTH, this.y + this.height / 2 - ENEMY_PROJECTILE_TARGET_HEIGHT / 2));
                AudioManager.playSound('enemyShootLight', false, 0.5); // NUOVO: Suono sparo
            }
        }
    }
    draw() {
        let currentAnim = this.isShooting && this.shootingAnimation ? this.shootingAnimation : this.idleAnimation;
        let spriteToUse = currentAnim ? currentAnim.spritesheet : this.sprite;
        const spriteUsable = spriteToUse && spriteToUse.complete && spriteToUse.naturalWidth > 0;
        const frameWidthToUse = currentAnim ? currentAnim.frameWidth : ENEMY_FOUR_ACTUAL_FRAME_WIDTH;
        const frameHeightToUse = currentAnim ? currentAnim.frameHeight : ENEMY_FOUR_ACTUAL_FRAME_HEIGHT;

        if (currentAnim && spriteUsable) {
            const f = currentAnim.getFrame();
            ctx.drawImage(spriteToUse, f.sx, f.sy, f.sWidth, f.sHeight, this.x, this.y, this.width, this.height);
        } else if (this.sprite && this.sprite.complete && this.sprite.naturalWidth > 0) {
             ctx.drawImage(this.sprite, 0,0, frameWidthToUse, frameHeightToUse, this.x, this.y, this.width, this.height);
        } else {
            ctx.strokeStyle = this.fallbackColor; ctx.lineWidth = lineWidth;
            ctx.strokeRect(this.x, this.y, this.width, this.height);
        }
        if(this.maxHealth>1&&this.health>0&&this.health<this.maxHealth){const hbW=this.width*.8,hbH=5,hbX=this.x+(this.width-hbW)/2,hbY=this.y-hbH-3;ctx.fillStyle='rgba(100,100,100,0.7)';ctx.fillRect(hbX,hbY,hbW,hbH);ctx.fillStyle='rgba(0,255,0,0.7)';ctx.fillRect(hbX,hbY,hbW*(this.health/this.maxHealth),hbH);}
    }
...
class EnemyProjectile{constructor(x,y){this.x=x;this.y=y;this.width=ENEMY_PROJECTILE_TARGET_WIDTH;this.height=ENEMY_PROJECTILE_TARGET_HEIGHT;this.speed=ENEMY_PROJECTILE_SPEED;this.sprite=images['enemyFourProjectile'];this.animation=null;if(this.sprite&&this.sprite.complete&&this.sprite.naturalWidth>0&&ENEMY_PROJECTILE_NUM_FRAMES>1){this.animation=new Animation(this.sprite,ENEMY_PROJECTILE_ACTUAL_FRAME_WIDTH,ENEMY_PROJECTILE_ACTUAL_FRAME_HEIGHT,ENEMY_PROJECTILE_NUM_FRAMES);}else if(this.sprite && (!this.sprite.complete || this.sprite.naturalWidth === 0)){console.warn("Sprite proiettile nemico non caricato o rotto.");}}update(dt){this.x-=this.speed*dt;if(this.animation)this.animation.update(dt);}draw(){const spriteUsable=this.sprite&&this.sprite.complete&&this.sprite.naturalWidth>0;if(this.animation&&spriteUsable){const f=this.animation.getFrame();ctx.drawImage(this.sprite,f.sx,f.sy,f.sWidth,f.sHeight,this.x,this.y,this.width,this.height);}else if(spriteUsable){ctx.drawImage(this.sprite,0,0,ENEMY_PROJECTILE_ACTUAL_FRAME_WIDTH,ENEMY_PROJECTILE_ACTUAL_FRAME_HEIGHT,this.x,this.y,this.width,this.height);}else{ctx.fillStyle=enemyProjectileColor;ctx.fillRect(this.x,this.y,this.width,this.height);}}}
...
class ArmoredShootingEnemy extends BaseEnemy {
    constructor(x, y) {
        super(x, y, ENEMY_SIX_TARGET_WIDTH, ENEMY_SIX_TARGET_HEIGHT, 'enemySix', ENEMY_SIX_ACTUAL_FRAME_WIDTH, ENEMY_SIX_ACTUAL_FRAME_HEIGHT, ENEMY_SIX_IDLE_NUM_FRAMES, 0.4, ARMORED_SHOOTING_ENEMY_HEALTH, armoredShootingEnemySpawnColor, 60); // MODIFICATO: Punteggio
        this.idleAnimation = this.animation;
        this.isShooting = false;
    ...
    update(dt) {
        this.x -= this.speed * dt;
        if (this.idleAnimation) { this.idleAnimation.update(dt); }
        this.shootTimer += dt;
        if (this.shootTimer >= ARMORED_SHOOTING_ENEMY_SHOOT_INTERVAL) {
            this.shootTimer = 0;
            enemyProjectiles.push(new EnemyProjectile(this.x - ENEMY_PROJECTILE_TARGET_WIDTH, this.y + this.height / 2 - ENEMY_PROJECTILE_TARGET_HEIGHT / 2));
            AudioManager.playSound('enemyShootHeavy', false, 0.6); // NUOVO: Suono sparo
        }
    }
}
...
class ToughBasicEnemy extends BaseEnemy {
    constructor(x, y) {
        super(x, y, ENEMY_SEVEN_TARGET_WIDTH, ENEMY_SEVEN_TARGET_HEIGHT, 'enemySeven', ENEMY_SEVEN_ACTUAL_FRAME_WIDTH, ENEMY_SEVEN_ACTUAL_FRAME_HEIGHT, ENEMY_SEVEN_NUM_FRAMES, 0.6, TOUGH_BASIC_ENEMY_HEALTH, toughBasicEnemySpawnColor, 30); // MODIFICATO: Punteggio
    }
}

class DangerousFlyingEnemy extends FlyingEnemy {
    constructor(x, y) {
        // Chiamata a super() di FlyingEnemy, che a sua volta chiama BaseEnemy
        super(x, y); // FlyingEnemy imposterà già sprite 'enemyFive', scoreValue etc.
                      // Dobbiamo sovrascrivere qui le proprietà specifiche per DangerousFlyingEnemy

        this.sprite = images['dangerousFlyingEnemy']; // Sovrascrive sprite
        ...
        this.scoreValue = 150; // Sovrascrive scoreValue
...
// NUOVO: Classe Miniboss Glitchzilla
class Glitchzilla extends BaseEnemy {
    constructor(x, y) {
        super(x, y, GLITCHZILLA_TARGET_WIDTH, GLITCHZILLA_TARGET_HEIGHT, 'glitchzilla', GLITCHZILLA_ACTUAL_FRAME_WIDTH, GLITCHZILLA_ACTUAL_FRAME_HEIGHT, GLITCHZILLA_NUM_FRAMES, 0.2, GLITCHZILLA_HEALTH, '#FF00FF', GLITCHZILLA_SCORE_VALUE); // Colore fallback magenta
    ...
    update(dt) {
        super.update(dt); // Movimento base (lento)

        // Logica di attacco (esempio semplice)
        this.attackPatternTimer += dt;
        if (this.attackPatternTimer > 3) { // Ogni 3 secondi tenta un attacco
            this.attackPatternTimer = 0;
            this.currentAttackPhase = (this.currentAttackPhase + 1) % 2;

            if (this.currentAttackPhase === 0) { // Attacco 1: Raffica di proiettili
                console.log("Glitchzilla: Raffica!");
                AudioManager.playSound('glitchzillaAttack', false, 0.7);
                for (let i = 0; i < 3; i++) {
                    setTimeout(() => {
                        enemyProjectiles.push(new EnemyProjectile(this.x - ENEMY_PROJECTILE_TARGET_WIDTH, this.y + (this.height / 3) * (i + 0.5) - ENEMY_PROJECTILE_TARGET_HEIGHT / 2));
                    }, i * 150);
                }
            } else { // Attacco 2: Proiettile grande o diverso (da implementare se vuoi un altro tipo di proiettile)
                console.log("Glitchzilla: Attacco speciale (placeholder)!");
                 AudioManager.playSound('glitchzillaAttack', false, 0.8);
                enemyProjectiles.push(new EnemyProjectile(this.x - ENEMY_PROJECTILE_TARGET_WIDTH, this.y + this.height / 2 - ENEMY_PROJECTILE_TARGET_HEIGHT / 2)); // Usa proiettile normale per ora
            }
        }

        // Impedisci al miniboss di uscire troppo dallo schermo a sinistra (esempio)
    ...
    takeDamage(dmg = 1) {
        ...
        if (this.health <= 0) {
            ...
            // Logica aggiuntiva per la sconfitta (es. drop speciale, fine del livello boss)
            activeMiniboss = null; // Rimuovi riferimento al boss attivo
            hasGlitchzillaSpawnedThisGame = true; // Segna che è stato sconfitto per non farlo riapparire subito (o gestisci diversamente)
        }
    }
}
...
// MODIFICATO: Non spawnare nemici normali se c'è un miniboss
...
// NUOVO: Logica spawn Glitchzilla
function spawnGlitchzillaIfNeeded() {
    if (!activeMiniboss && !hasGlitchzillaSpawnedThisGame && score >= GLITCHZILLA_SPAWN_SCORE_THRESHOLD) {
        ...
        hasGlitchzillaSpawnedThisGame = true; // Per ora, spawna solo una volta per partita
                                          // Potresti resettare questo in resetGame se vuoi che appaia ogni volta dopo la soglia
                                          // o implementare logica per farlo riapparire dopo un po'
    }
}
...
function spawnPowerUpAmbientIfNeeded(dt) {
    ...
    if (powerUpSpawnTimer >= nextPowerUpSpawnTime) {
        ...
        if (availablePowerUps.length === 0) {
             console.log("Nessun power-up non conflittuale disponibile per lo spawn.");
             return;
        }
        ...
        powerUpItems.push(new PowerUpItem(canvas.width, spawnY, randomType, images, () => gameSpeed)); // Passa gameSpeed come funzione
...
function updateAllEnemyTypes(dt){
    ...
    // NUOVO: Update Miniboss
    if (activeMiniboss) {
        activeMiniboss.update(dt);
        if (activeMiniboss.health <= 0) { // Controlla se è stato sconfitto durante l'update
            activeMiniboss = null; // Rimosso qui se sconfitto
        } else if (activeMiniboss.x + activeMiniboss.width < 0) { // Se esce dallo schermo
            activeMiniboss = null;
        }
    }
}
    ...
    if(!asyncDonkey.isShieldActive){
        const checkPlayerCollisionWithEntity=(entity, isObstacle = false)=>{
            if (isObstacle && asyncDonkey.isFirewallActive) {
                // NUOVO: Suono per collisione non letale con Firewall
                if(entity&&pC.x<entity.x+entity.width&&pC.x+pC.width>entity.x&&pC.y<entity.y+entity.height&&pC.y+pC.height>entity.y){
                    // AudioManager.playSound('firewallDeflect'); // Aggiungere suono se si vuole
                }
                return false; // Firewall attivo, nessuna collisione dannosa con ostacoli
            }
            if(entity&&pC.x<entity.x+entity.width&&pC.x+pC.width>entity.x&&pC.y<entity.y+entity.height&&pC.y+pC.height>entity.y){
        ...
        if(checkPlayerCollisionWithList(armoredEnemies))return; if(checkPlayerCollisionWithList(shootingEnemies))return; if(checkPlayerCollisionWithList(enemyProjectiles))return;
    ...
    for(let i=projectiles.length-1;i>=0;i--){
        ...
        const processProjectileHitOnList=(enemyList, indexInArray)=>{
            ...
            if(e&&p.x<e.x+e.width&&p.x+p.width>e.x&&p.y<e.y+e.height&&p.y+p.height>e.y){
                ...
                if(e.health<=0){
                    AudioManager.playSound('enemyExplode', false, 0.7);
                    score+= e.scoreValue; // Usa scoreValue dell'istanza nemico
                ...
                }else if(e instanceof ArmoredEnemy || e instanceof ArmoredShootingEnemy){score+=5;} // Punti per colpo su corazzato
                return true;
            }return false;
        };
        // NUOVO: Collisione Proiettili vs Miniboss
        if (activeMiniboss && p.x < activeMiniboss.x + activeMiniboss.width && p.x + p.width > activeMiniboss.x && p.y < activeMiniboss.y + activeMiniboss.height && p.y + p.height > activeMiniboss.y) {
            ...
            if (activeMiniboss.health <= 0) {
                ...
                // activeMiniboss è gestito (messo a null) nel suo metodo takeDamage o in updateAllEnemyTypes
            }
        }
        if(projectileConsumedThisHit){projectiles.splice(i,1);continue;}
...
function resetGame(){
    ...
    activeMiniboss = null; // NUOVO
    hasGlitchzillaSpawnedThisGame = false; // NUOVO
...
function updatePlaying(dt){
    ...
    // NUOVO: Spawning e updates Miniboss prima degli altri nemici
    spawnGlitchzillaIfNeeded();
    if (activeMiniboss) { // Se il miniboss è attivo, solo lui e il player vengono aggiornati (più proiettili)
        updateProjectiles(dt); // Proiettili del giocatore
        enemyProjectiles.forEach((ep,i)=>{ep.update(dt);if(ep.x+ep.width<0)enemyProjectiles.splice(i,1);}); // Proiettili nemici (inclusi quelli del boss)
        if(activeMiniboss) activeMiniboss.update(dt); // Aggiorna il boss
         // Non spawnare/aggiornare altri nemici o ostacoli
    } else { // Logica normale se non c'è miniboss
        ...
        updateAllEnemyTypes(dt);spawnFlyingEnemyIfNeeded(dt);
...
function drawPlayingScreen(){
    ...
    drawGround();
    // MODIFICATO: Disegna ostacoli solo se non c'è miniboss attivo o gestisci diversamente
    if (!activeMiniboss) {
        drawObstacles();
    }
    drawAllEnemyTypes(); // Includerà il miniboss se activeMiniboss non è null
